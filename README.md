...
[![CI](https://github.com/sjourdois/cooldeadpipewire/actions/workflows/ci.yml/badge.svg)](https://github.com/sjourdois/cooldeadpipewire/actions/workflows/ci.yml)
...
> `com.cooldeadpipewire.sdPlugin`, and the `.sdPlugin` bundle is a
...
| **Active Application Volume** | Keypad · Encoder | Adjust the volume of the application owning the currently focused KDE/Wayland window |
| **Output Device** | Keypad · Encoder | Switch the system default output; pick several devices to cycle through them, with a per-device icon, or toggle a subset and grey out when another device is active; can retarget a single stream instead of the default |
...
The **Active Application Volume** action follows the focused KDE/Wayland window: a bundled KWin script reports its PID over the session D-Bus, and the plugin matches that PID against PipeWire's `application.process.id`. This works for native apps and Proton/Wine processes, including streams whose `application.name` is an `.exe`.
...
2. Assemble the plugin directory `com.cooldeadpipewire.sdPlugin/`:
   ```
   com.cooldeadpipewire.sdPlugin/
   ├── manifest.json            (from assets/)
   ├── icons/                   (from assets/)
   ├── propertyInspector/       (from assets/)
   └── <target-triple>/bin/cooldeadpipewire
   ```
   e.g. the binary at `x86_64-unknown-linux-gnu/bin/cooldeadpipewire`.
3. Copy that directory into `~/.config/opendeck/plugins/` and restart OpenDeck.

### KDE Plasma / Wayland active-application support

The **Active Application Volume** action uses a KWin script because Wayland does not expose the focused application through X11 APIs. KDE documents packaged KWin scripts and the `kpackagetool6` / `kwriteconfig6` installation flow here: [KWin scripting tutorial](https://develop.kde.org/docs/plasma/kwin/).

After installing the plugin source, run from the repository root:

```bash
./install-kwin-active-window.sh
```

This installs `kwin/opendeck-active-window`, enables it, and asks KWin to reconfigure. The script reports the active window PID to the plugin over the user session D-Bus. If you do not use KDE/KWin, this action is intentionally inactive rather than falling back to X11 or shell-based window detection.
...
cargo build --release      # -> target/release/cooldeadpipewire

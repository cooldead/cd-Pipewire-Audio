...
fn on_global(
	...
	match global.type_ {
		ObjectType::Node => {
			...
			let process_id = props.get("application.process.id").and_then(|s| s.parse::<u32>().ok());
    ...
    process_id,
			...
			inner.borrow_mut().nodes.insert(
				...
				NodeRef {
					...
					process_id: if is_stream { process_id } else { None },
...
fn on_command(inner: &Rc<RefCell<Inner>>, cmd: Command) {
	...
	match &cmd {
		...
		Command::AdjustAppPidVolume(pid, delta) => {
			let b = inner.borrow();
			let mut matched = 0;
			for n in b.nodes.values().filter(|n| n.is_stream && n.process_id == Some(*pid)) {
				let target = (n.volume_cubic + delta).clamp(0.0, 1.5);
				log::debug!("Adjusting PID {pid} stream '{}' node volume {} -> {}", n.name, n.volume_cubic, target);
				set_node_props(&n.proxy, Some(cubic_to_linear(target)), None);
				matched += 1;
			}
			if matched == 0 {
				...
				log::debug!("Adjusted {matched} audio stream(s) for PID {pid}");
			}
			return;
		}
		Command::SetAppPidMute(pid, value) => {
			...
			let mute = value.unwrap_or(!currently_muted);
			for n in &streams {
				log::debug!("Setting PID {pid} stream '{}' mute={mute}", n.name);
				set_node_props(&n.proxy, None, Some(mute));
			}
			if streams.is_empty() {
				log::warn!("No active audio streams for PID {pid}");
